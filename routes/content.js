  ...
  try {
    ...
    if (files.length > maxFilesPerType * 2) {
      return res
        .status(400)
        .json({
          message: `Exceeded the maximum number of files allowed (${maxFilesPerType} images + ${maxFilesPerType} videos)`,
        });
    }
    ...
    for (const image of images) {
      ...
      if (image && image.mimetype && image.data) {
        ...
        try {
          const { secure_url: imageUrl, public_id: imageCldId } =
            await uploadToCloudinary(
              base64Image,
              `contents/images/${imageFileName}`
            );
    ...
    for (const video of videos) {
      ...
      if (video && video.mimetype && video.data) {
        ...
        const base64Video = `data:${
          video.mimetype
        };base64,${video.data.toString("base64")}`;

        try {
          const { videoUrl, videoCldId } = await uploadVideoToCloudinary(
            base64Video,
            `contents/videos/${videoFileName}`
          );
...
const uploadVideoToCloudinary = async (base64Video, folderPath) => {
  try {
    const { secure_url: videoUrl, public_id: videoCldId } =
    ...
    if (error.name === "TimeoutError") {
      ...
      return res
        .status(500)
        .json({ message: "Cloudinary upload timed out. Please try again." });
    } else {
...
contentRouter.delete(
  ...
  async (req, res) => {
    try {
      ...
      const user =
        (await Admin.findById(userId)) || (await SuperAdmin.findById(userId));
...
contentRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      const replyAuthor =
        user.firstName && user.lastName ? `${firstName} ${lastName}` : userName;
...
contentRouter.get(
  "/contents/:contentId/comments/:commentId/replies/:replyId",
  verifyToken,
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      comment.replies = comment.replies.filter(
        (r) => r.toString() !== req.params.replyId
      );
      // Remove the reply from the comment's replies array
      ...
      if (result.deletedCount > 0) {
        ...
        res
          .status(500)
          .json({ message: "Internal Server Error: Unable to delete reply" });
      }
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Internal Server Error" });
